...
```python
def main(bit_fields):
    # Словарь, содержащий смещения и размеры для каждого битового поля
    bit_positions = {
        'H1': (0, 6),   # H1 занимает биты с 0 по 5 (6 бит)
        'H3': (13, 7),  # H3 занимает биты с 13 по 19 (7 бит)
        'H4': (21, 4),  # H4 занимает биты с 21 по 24 (4 бит)
        'H5': (24, 10), # H5 занимает биты с 24 по 33 (10 бит)
        'H6': (34, 8)   # H6 занимает биты с 34 по 41 (8 бит)
    }
    
    # Начальное значение результирующего числа
    result = 0
    
    # Обрабатываем каждое битовое поле
    for name, value in bit_fields:
        # Преобразуем значение из шестнадцатеричной строки в целое число
        int_value = int(value, 16)
        
        # Получаем смещение и размер для текущего битового поля
        offset, size = bit_positions[name]
        
        # Сдвигаем значение на нужное смещение и добавляем его к результату
        result |= (int_value << offset)
